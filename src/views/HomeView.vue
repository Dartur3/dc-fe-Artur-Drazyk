<template>
  ...
  <button v-if="resultNames && resultNames.characters.info && page < resultNames.characters.info.count / 20" type="button" class="droptxt" v-on:click="setPage(++page)">+</button>
  <button v-else type="button" class="droptxt">+</button>

  <SearchBar v-on:pass-search-bar-data="passSearchBarData" v-on:initialize-input="setName(id.name), setNameId(id.nameId), setEpisodeId(id.episode), setPage(page)"/>
  ...
  <div v-if="buttonClicked.name">
    ...
    <div v-else-if="resultNames && resultNames.characters.results" v-for="character in resultNames.characters.results"
      :key="character.id">
